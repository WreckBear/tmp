...
> 注意CHAR和VARCHAR的区别
>
> * CHAR代表是不可变长度的字符串，用的时候必须用括号数字来指定长度，例如CHAR(20)，长度最长255，如果该字段只放了2个字符，那么CHAR(20)类型的字段会用空格补全剩下18个字符位置
...
数组在内存中是一块连续的空间，而链表是有节点组成，在内存空间上并不连续，而是靠着每一个节点中的next指针链起来的一个集合。

数组的有点就是索引快，效率高，可以通过索引下表直接计算出元素的偏移地址；缺点是需要占用连续的整块内存空间，在内存有限的情况下，数组能力就有限了。

链表的有点就是可以利用零散的内存空间组成集合，灵活性更大；缺点是查找效率慢，因为如果你要找一个元素，需要从头节点顺着next指针一个个的找才能找到。
...
设置外键有多种方式：
<br/>
1. 在创建表时可以在某个字段的后面加上reference table_name(key)来指定外键并关联到table_name表的key字段上
